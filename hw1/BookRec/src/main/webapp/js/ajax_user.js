...
document.getElementById("ajaxButton")
    .addEventListener("click", searchUserByName);
...
function searchUserByName() {
    ...
    const salary = document.getElementById("nameID").value;

    console.log("name threshold: %s.", name);

    const url = "http://localhost:8080/BookRec/rest/user/name/" + name;
...
function processResponse(xhr) {
    ...
    for (let i = 0; i < resourceList.length; i++) {
        ...
        eee.appendChild(document.createTextNode(user["name"]));
        ee.appendChild(eee); // append the cell to the row

        // create a cell for the surname of the employee
        eee = document.createElement("td");
        eee.appendChild(document.createTextNode(user["email"]));
        ee.appendChild(eee); // append the cell to the row

        // create a cell for the age of the employee
        eee = document.createElement("td");
        eee.appendChild(document.createTextNode(user["password"]));
        ee.appendChild(eee); // append the cell to the row
